...
<body>
	<div class="main">
		...
		<div class="main5">
			<!--toolbar-->
			<input type="button" onclick="doSub2();"
				style="background:url(img/t-save.png) no-repeat; width:64px; height:25px; border:none; margin-bottom:10px;" />

			<!--form-->
			<form id="form" class="registerform" action="storetype!addBackInMaterial.action">
				<input type="hidden" name="materialLog.mt_id" value="${mt_id}" />
				<input type="hidden" name="materialLog.pro_name" id="pro_name" />
				...
				<table
					...
					<tr>
						<th><span class="need">*</span> 工程名:</th>
						<td>
							<input type="text" readonly="readonly" onclick="selectProject();"
							id="pro_id" name="materialLog.pro_id" class="inputxt" datatype="*1-100" nullmsg="工程名不可为空！" />
							<div class="Validform_checktip">工程名为返库材料使用的工程，不可为空！</div>
						</td>
					</tr>
					
					<tr>
						<th style="width:35%;"><span class="need">*</span> 返库数量:</th>
						<td style="width:65%;">
					...
					<tr>
						...
						<td><input type="text" readonly="readonly" onclick="$.calendar({ maxDate:'%y-%M-%d' });"
							id="ml_date" name="materialLog.ml_date" class="inputxt"  errormsg="日期不可为空！" />
