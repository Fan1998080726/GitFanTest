		   ...
		   <div class="toolbar">
		    <form method="post" id="userform">
		      <ul class="clearfix">
		        ...
		        <li>
		          <input type="button" onclick="updateChazhao();"
			...
			<!--form-->
			<form id="form" class="registerform" action="storetype!editMaterialApply.action">
				<input type="hidden" name="materialApply.ma_flag" value="0">
				<input type="hidden" id="ma_id" name="materialApply.ma_id" value="${materialApply.ma_id}">
				<!--table-->
				<table
					...
					<tr>
						...
						<td style="width:25%;">
							<input type="hidden" id="mt_id" value="${materialApply.mt_id}" name="materialApply.mt_id" />
							<input type="text" id="mt_name" readonly="readonly"	class="inputxt"
									datatype="*1-20" value="${materialApply.mt_name}" errormsg="材料名不可为空！" />
						...
						<td style="width:25%;">
							<input type="text" id="ma_num" 
									name="materialApply.ma_num" value="${materialApply.ma_num}" class="inputxt"  onblur="calculate();"
									datatype="/^\d{1,8}(?:\.\d{0,2})?$/" nullmsg="数量不可为空!" errormsg="数量不可为空，只能输入数字！" />
					...
					<tr>
						...
						<td>
							<input type="text" id="ma_unit" readonly="readonly"
									name="materialApply.ma_unit" value="${materialApply.ma_unit}" class="inputxt"
									datatype="*1-20" errormsg="单位不可为空！" />
							<div class="Validform_checktip">单位不可为空！</div>
						</td>
						
						<th><span class="need">*</span> 日期:</th>
						<td><input type="text" readonly="readonly" onclick="$.calendar();" value="${materialApply.ma_apply_date}"
					...
					<tr>
						...
						<td>
							<input type="text" id="ma_price" 
									name="materialApply.ma_price" value="${materialApply.ma_price}" onblur="calculate();" class="inputxt"
									datatype="/^\d{1,8}(?:\.\d{0,2})?$/" errormsg="单价不可为空,可有两位小数八位整数！" />
							<div class="Validform_checktip">单价为材料价格，不可为空，可有两位小数八位整数！</div>
						</td>
						
						<th> 总价（元）:</th>
						<td><input type="text" readonly="readonly" id="ma_cost"  class="inputxt" value="<fmt:formatNumber value="${materialApply.ma_price*materialApply.ma_num}" pattern="##.##" minFractionDigits="2" ></fmt:formatNumber>" />
						</td>
					</tr>
				</table>
