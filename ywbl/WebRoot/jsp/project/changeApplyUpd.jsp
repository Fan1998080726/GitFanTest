...
<script type="text/javascript" src="<%=path%>/jsp/project/changeApplyUpd.js"></script>
</head>
...
<body>
	<div class="main">
		<div class="main5">
			<!--toolbar-->
			<input type="button" onclick="doSub('1','${changeApply.aaId}');" value=""
				style="background:url(<%=path%>/img/t-zancun.png) no-repeat; width:64px; height:25px; border:none; margin-bottom:10px;" />
				<input type="button" onclick="doSub('2','${changeApply.aaId}');" value=""
			...
			<form id="form" action="" class="registerform" method="post">
				...
				<table
						...
						<td><input name="" type="button" value="上传附件"
							...
							<table id="fileUploadTable" class="table2 table2-striped table2-bordered table2-condensed table2-down">
								<c:forEach items="${changeApply.files}" var="files">
									<tr>
										<td>${files.cafName}
											<input type='hidden' value='${files.cafName}' name='fileName'/>
											<input type='hidden' name='filePath' value='${files.cafUrl }'/>
										</td>
										<td>
											<input type='button' value='删除' onclick='javascript:$(this).parent().parent().remove()'/>
										</td>
									</tr>
								</c:forEach>
							</table>
					...
					<tr>
						<th>备注：</th>
						<td><textarea id="remark" name="remark" rows="5" cols="5">${changeApply.aaRemark}</textarea>
						</td>
					</tr>
				</table>
			</form>
