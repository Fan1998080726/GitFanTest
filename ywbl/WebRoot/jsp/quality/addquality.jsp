...
<meta charset="utf-8">
	...
	function doSub() {
		...
		if(null != $('#qc_file_name').val() && 0==$('#uploadfile').val().length){
			$(".registerform").submit();
		}else{
			ajaxFileUpload();
		}
	...
	function ajaxFileUpload(){
	        ...
	        (
	            {
	                url:'quality!ajaxFileUpload.action',
	                secureuri:false,
	                ...
	                {   
	                	if(data.status=='y'){
							$('#qc_file_url').val(data.qc_file_url);
							$('#qc_file_name').val(data.qc_file_name);
							$(".registerform").submit();
	                	}else{
	                		$.alert(data.info);
	                		return false;
...
<table class="table2 table2-striped table2-bordered table2-condensed table2-down">
  <tr>
    ...
    <td style="width:65%;">
    					<input type="hidden" id="pc_id" name="quality_control.pc_id" value="${quality_control.pc_id }"/>
    					<input type="hidden" id="pc_name" name="quality_control.pc_name" value="${quality_control.pc_name }"/>
    					<input type="hidden" id="qc_id" name="quality_control.qc_id" value="${quality_control.qc_id }"/>
						<input type="text" id="qc_title" name="quality_control.qc_title" value="${quality_control.qc_title}" class="inputxt" datatype="*1-20" errormsg="标题不可为空,最多20个字符！" />
							<div class="Validform_checktip">标题不可为空,最多20个字符！</div></td>
  </tr>
  <tr>
    <th><span class="need">*</span> 类型：</th>
    <td><select id="qc_type" name="quality_control.qc_type">
									<option value="" style="font-size:12px;">请选择</option>
									<option value="27" <c:if test="${quality_control.qc_type==27}">selected="selected"</c:if> style="font-size:12px;">消息</option>
									<option value="28" <c:if test="${quality_control.qc_type==28}">selected="selected"</c:if> style="font-size:12px;">报警</option>
							</select>
  ...
  <tr>
    ...
    <td><input id="uploadfile" type="file" name="uploadfile" />
							<input type="hidden" id="qc_file_name" name="quality_control.qc_file_name" value="${quality_control.qc_file_name}" />
							<input type="hidden" id="qc_file_url" name="quality_control.qc_file_url" value="${quality_control.qc_file_url}" />
							<c:if test="${quality_control.qc_file_name!=null}">
								<a href="javascript:fileDownload('${quality_control.qc_file_name }','${quality_control.qc_file_url }');" style="color:#014265;">${quality_control.qc_file_name}</a>
							</c:if>
							</td>
  </tr>
  <tr>
    <th><span class="need"></span> 描述：</th>
    <td><textarea id="qc_remark" name="quality_control.qc_remark" rows="5" cols="5"><c:if test="${quality_control.qc_remark!=null}">${quality_control.qc_remark}</c:if></textarea>
						</td>
  </tr>
</table>
