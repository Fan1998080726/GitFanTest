	...
	<div class="main">
		<div class="main5">
         ...
         <!--toolbar-->
			...
			<!--form-->
			<form id="form" class="registerform" action="cost!insertUpdate.action">
			<input type="hidden" name="costActual.ca_id" value="${ca_id }"/>
...
<c:if test="${'update'==type }">
  <tr>
    ...
    <td style="width:30%;"><input type="text" id="login_id" name="costActual.ca_value" class="inputxt" datatype="/^\d{1,8}(?:\.\d{0,2})?$/" nullmsg="消耗值不可为空，可有两位小数八位整数！" errormsg="消耗值不可为空，可有两位小数八位整数！" />
		                    <div class="Validform_checktip">消耗值不可为空，可有两位小数八位整数！</div></td>
  </tr>
</c:if>
  <tr>
    <th style="width:20%;"><span class="need">*</span> 备注:</th>
    <td style="width:30%;"><input type="text" id="login_id" name="costActual.ca_remark" class="inputxt" datatype="*1-50" errormsg="备注不可为空，最多50字符！" />
