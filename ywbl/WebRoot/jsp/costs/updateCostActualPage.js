$(function(){
		...
		$("input[name='costActual.cp_id']").each(function (){
			$(this).attr('value',data.cp_id);
			});
		//$('#cp_name').val(data.cp_name);
		if(data.type_id==2||data.type_id==3){
		$("input[name='costActual.cp_name']").each(function (){
			$(this).attr('value',data.plan_name);
			});
		}else{
			$("input[name='cp_name']").each(function (){
				$(this).attr('value',data.plan_name);
				});
		}
		//$('#cp_type').val(data.cp_type);
		$("input[name='type_id']").each(function (){
			$(this).attr('value',data.type_name);
			});
