...
<body>
<div class="main">
  <div class="breadcrumb">当前位置：事项管理 
<!--   疫情防控检查-项目检查 -->
    ...
    <form action="" method="post" name="prjcheckinfoform" 	class="prjcheckinfoform"	id="prjcheckinfoform">
  	...
  	<input value="${builyear}"   name="builyear"  type="hidden"/>
         ...
         <c:if test="${opstate=='1' }">
             ...
             <c:if test="${opstate!='1' }">
         <li>
            <span>建设单位：</span><input name="BuildCorpName" id="BuildCorpName" value="${ BuildCorpName }" type="text" class="span3" placeholder="请输入文字...">
        </li>
         <li>
            <span>施工单位：</span><input name="ConsCorpName" id="ConsCorpName" value="${ ConsCorpName }" type="text" class="span3" placeholder="请输入文字...">
        </li>
         <li>
            <span>监理单位：</span><input name="SuperCorpName" id="SuperCorpName" value="${ SuperCorpName }" type="text" class="span3" placeholder="请输入文字...">
        </li>
        </c:if>
         </c:if>
      </ul>	
...
<%--     	         <c:if test=" ${dept_name!='企业用户组' }"> --%>
      ...
      <input type="button" name="btn_add" onclick="updatePrjcheckinfoPage('${ builyear }')" value="" style="background:url(img/t-add.png) no-repeat;"/>
     <input type="button" name="btn_update2" onclick="exportExcel()" value="" style="background:url(img/t-down.png) no-repeat;"/>
      <input type="button" name="btn_del" onclick="del('${ PrjNum }')" value="" style="background:url(img/t-delete.png) no-repeat;"/>
...
<!-- 	                 保存时 -->
				<a  href="matters!updateMattersPage.action?mattersVo.id=${m.id }">修改</a>/
	                 <c:if test="${m.state=='01'}">
								<a  href="#" 	onclick="MattersDel('${m.id}','${m.matters_id}')" >删除</a>
	                 </c:if>
...
<!-- 				还没有做提前办结	            -->
	           <c:if test="${m.state!='06'}">
				<a  href="#"   onclick="endState('${m.id}')" >办结</a>
	           </c:if>      
<!-- 				提前办结了 -->
	           <c:if test="${m.state=='06'}">
				已办结
