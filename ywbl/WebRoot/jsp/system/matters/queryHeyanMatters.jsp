...
function  AuditMatters(id,cid,cstate){
	 layer.open({
 		  type: 2,
 		  title:'事项审核',
 			setOpaqueRate:'1',
 		  ...
 		  fixed: true, //不固定
 		  maxmin: true,
 		  content: 'feedback!queryAdminFeedbackMatters.action?id='+id+"&cid="+cid+"&cstate="+cstate
 		}); 
}
...
<!--     </div> -->
 ...
 <div align="right"	>
     <button type="button" 	onclick="ReturnURl()"	class="layui-btn layui-btn-normal">  <i class="layui-icon"  style="font-size: 18px">&#xe65c;</i> 返回</button>
     </div>
            ...
            <tbody>
            	<c:forEach items="${list}" var="m" varStatus="stauts">
	              <tr <c:if test="${stauts.count%2==1 }">class="odd"</c:if>  >  
	                 ...
	                 <td   >
	                 	 				<a  href="#"   onclick="AuditMatters('${m.id}','${m.cid}','${m.cstate}')"    class="layui-btn layui-btn-normal">
	                 	        <span class="iconfont"  style="margin-left:5px">&#xe66c;</span>&nbsp;审核</a>
