...
function UndoMatters(id){
	 layer.confirm('是否确认恢复该条事项?', {
	       btn: ['确定', '取消']
	   }, function (index, layero) {
			$.ajax({
		        type: "post",
		        url: "matters!UndoMatters.action?id="+id,
		        success: function(data){
		        	   //无法关闭这个消息框
		            layer.closeAll('dialog');  //加入这个信息点击确定 会关闭这个消息框
...
<!-- 	                 保存时 -->
				<a  href="#"  onclick="UndoMatters('${m.id}')"  class="layui-btn layui-btn-normal">
...
function chaxun(){
	$('#prjcheckinfoform').attr("action","matters!queryDeleteMatters.action");
