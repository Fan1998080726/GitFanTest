		...
		function onheyan(){
			 layer.confirm('是否确认进行核验申请?', {
					...
					$.ajax({
				        type: "post",
				    	data : $(".layui-form").serialize(),  
				        url: "feedback!RequestCheck.action",
				        success: function(data){
				        	   //无法关闭这个消息框
// 				            layer.closeAll('dialog');  //加入这个信息点击确定 会关闭这个消息框
				            fnIntoView();
...
// 			签收
			function onqianshou(){
			 	 ...
			 	 layer.open({
			    		...
			    		fixed: true, //不固定
			  		  maxmin: true,
			  		  content: 'matters!IntoSigInPage.action?mattersid='+'${mattersVo.matters_id}'
			  		}); 
			}
...
<div align="right"	>
	...
	<c:if test="${delayState=='1'}">
	
	    <button type="button" 	onclick="queryDelayById()"	class="layui-btn layui-btn-normal">已提交申请请等待审核</button>
	
	</c:if>
	<c:if test="${delayState!='1'}">
		
		<c:if test="${mattersVo.yqstate=='03'}">（已退回请重新申请:${mattersVo.yqcontent}）</c:if>	
		
    <button type="button" 	onclick="sendDelay()"	class="layui-btn layui-btn-normal">申请延期</button>
    
	</c:if>
    
    
    <c:if test="${cstate=='01'}">
        <button type="button"	onclick="onqianshou()"	 class="layui-btn layui-btn-normal">签收</button>
    
	</c:if>
    <c:if test="${cstate!='01'}">
    <button type="button"	onclick="delFeedback()"	 cl	 class="layui-btn layui-btn-normal">删除反馈</button>
    <button type="button"	onclick="onheyan()"	 class="layui-btn layui-btn-normal">核验申请</button>
    <button type="button"	onclick="onFeedback()"	 class="layui-btn layui-btn-normal">反馈</button>
    </c:if>
 ...
 <div align="right"	>
   ...
   <c:if test="${hoursState=='0'}">
             <input type="checkbox"   lay-skin="primary"	value="${v.id }"	 name="checkId"   id="checkId"   >
                         <span style="color: red;" >*(删除限制时间为一小时)</span>
   </c:if>
   <c:if test="${hoursState!='0'}">
            <span style="color: red;" >*(删除限制时间为一小时，已超时)</span>
   </c:if>
    </div>
