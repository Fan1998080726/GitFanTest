...
<div align="right"	>
    ...
    <button type="button"	onclick="MattersAudit('1')"	 class="layui-btn layui-btn-normal">通过</button>
    <button type="button"	onclick="MattersAudit('2')"	 class="layui-btn layui-btn-normal">退回</button>
        &nbsp;&nbsp;&nbsp;&nbsp;
</div>
...
<%-- 	<c:forEach items="${feedList }"   var="v"	  begin="0"	end="0"	> --%>
      ...
      <textarea placeholder="请输入内容" 	style="width: 99%"	class="layui-textarea" name="endcontent"> </textarea>
  ...
  <tr>
    ...
    <td colspan="3">
              <c:forEach var="m" items="${companyfileList}">
					<a href="http://<%=request.getServerName()%>:<%=request.getServerPort() %>/dbxtdownload/${m.cf_name }" target="_bank">
 					<img src="http://<%=request.getServerName()%>:<%=request.getServerPort() %>/dbxtdownload/${m.cf_name }"  style="max-width:600px"   />
 					</a>
              </c:forEach>
									</td>
  </tr>
