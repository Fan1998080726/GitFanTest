...
//杨傲鹏 监理合同 PDf合同
jQuery.closeDialogjlht = function(data,dialogId){
	setTimeout(function () { //解决jquery ie9 bug date string 未定义
		/**
		 * 调用打开这个dialog的窗口的_closeDialog方法，传入data,和需要关闭的id
		 */
		art.dialog.opener._closeDialogjlht(data,dialogId);
	}, 0)
}
function _closeDialogjlht(data,dialogId){ 
	//当弹出多个dialog时候，每个dialog的id应该不同，所以关闭时应该传入id
	if(dialogId === undefined){
		art.dialog.list['_urlDialog'].close();
		getChildDatajlht(data);
	}else{
		art.dialog.list[dialogId].close();
		getChildDatajlht(data);
	}
...
//杨傲鹏 填报人员
jQuery.closeDialogtbry = function(data,dialogId){
	setTimeout(function () { //解决jquery ie9 bug date string 未定义
		/**
		 * 调用打开这个dialog的窗口的_closeDialog方法，传入data,和需要关闭的id
		 */
		art.dialog.opener._closeDialogtbry(data,dialogId);
	}, 0)
}
function _closeDialogtbry(data,dialogId){ 
	//当弹出多个dialog时候，每个dialog的id应该不同，所以关闭时应该传入id
	if(dialogId === undefined){
		art.dialog.list['_urlDialog'].close();
		getChildDatatbry(data);
	}else{
		art.dialog.list[dialogId].close();
		getChildDatatbry(data);
	}
...
//资质
jQuery.closeDialogzz = function(data,dialogId){
	setTimeout(function () { //解决jquery ie9 bug date string 未定义
		/**
		 * 调用打开这个dialog的窗口的_closeDialog方法，传入data,和需要关闭的id
		 */
		art.dialog.opener._closeDialogzz(data,dialogId);
	}, 0)
}
function _closeDialogzz(data,dialogId){ 
	//当弹出多个dialog时候，每个dialog的id应该不同，所以关闭时应该传入id
	if(dialogId === undefined){
		art.dialog.list['_urlDialog'].close();
		getChildDatazz(data);
...
//关闭 一地一码  fcl  2020年6月23日15:47:56
jQuery.closeDialog_queryPrjCheckInfo_selectPage = function(data,dialogId){
 
	setTimeout(function () { //解决jquery ie9 bug date string 未定义
		/**
		 * 调用打开这个dialog的窗口的_closeDialog方法，传入data,和需要关闭的id
		 */
		art.dialog.opener._closeDialog_queryprjcheckinfo_selectPage(data,dialogId);
	}, 0)
}


function _closeDialog_queryprjcheckinfo_selectPage(data,dialogId){ 
//	alert("id="+data);
	//当弹出多个dialog时候，每个dialog的id应该不同，所以关闭时应该传入id
	if(dialogId === undefined){
		art.dialog.list['_urlDialog'].close();
		getChildData_queryprjchekinfo_selectPage(data);
	}else{
		art.dialog.list[dialogId].close();
		getChildData_queryprjchekinfo_selectPage(data);
	}
...
//关闭 燃气企业  fcl  2021年3月16日16:25:46
jQuery.closeDialog_queryGasUser_selectPage = function(data,dialogId){
	
	setTimeout(function () { //解决jquery ie9 bug date string 未定义
		/**
		 * 调用打开这个dialog的窗口的_closeDialog方法，传入data,和需要关闭的id
		 */
		art.dialog.opener._closeDialog_queryGasUser_selectPage(data,dialogId);
	}, 0)
}


function _closeDialog_queryGasUser_selectPage(data,dialogId){ 
//	alert("id="+data);
	//当弹出多个dialog时候，每个dialog的id应该不同，所以关闭时应该传入id
	if(dialogId === undefined){
		art.dialog.list['_urlDialog'].close();
		getChildData_queryGasUser_selectPage(data);
	}else{
		art.dialog.list[dialogId].close();
		getChildData_queryGasUser_selectPage(data);
	}
...
//20170425 add 中标备案信息管理选择项目报建后，关闭页面
jQuery.closeDialog_ExaminePerson = function(data,dialogId){
	setTimeout(function () { //解决jquery ie9 bug date string 未定义
		/**
		 * 调用打开这个dialog的窗口的_closeDialog方法，传入data,和需要关闭的id
		 */
		art.dialog.opener._closeDialog_ExaminePerson(data,dialogId);
	}, 0)
}

function _closeDialog_ExaminePerson(data,dialogId){ 
	//当弹出多个dialog时候，每个dialog的id应该不同，所以关闭时应该传入id
	if(dialogId === undefined){
		art.dialog.list['_urlDialog'].close();
		getChildData_queryExaminePerson_selectPage(data);
	}else{
		art.dialog.list[dialogId].close();
		getChildData_queryExaminePerson_selectPage(data);
	}
